...
def benchmark_model(model_name, benchmark_dir, device="cpu"):
    ...
    model = BLIP_Base('configs/bert_config.json')
    model, _ = load_checkpoint(model, 'model_base.pth')
    model.cuda()
    ...
    with open(csv_file, "r") as f:
        ...
        for i, row in enumerate(reader):
            ...
            with torch.no_grad():
                output1 = model(imgs, pre_caption(text1), mode='multimodal')
                logits1 = model.itm_head(output1[:, 0, :])
                itm_score1 = torch.nn.functional.softmax(logits1, dim=1)[:, 1]
                # pred1 = torch.argmax(itm_score1).squeeze()

                output2 = model(imgs, pre_caption(text2), mode='multimodal')
                logits2 = model.itm_head(output2[:, 0, :])
                itm_score2 = torch.nn.functional.softmax(logits2, dim=1)[:, 1]
