...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    #metric_logger.add_meter('loss_attn', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    for i, (image, caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, loss_itm, loss_mlm, loss_mim = model(image, caption, alpha = alpha)  
        loss = loss_ita + loss_itm + loss_mlm +loss_mim
...
def main(args, config):
    ...
    model = blip_pretrain(mask_rate=args.mask_rate,prompt_length=args.prompt_length,image_size=config['image_size'], vit=config['vit'], vit_grad_ckpt=config['vit_grad_ckpt'], 
