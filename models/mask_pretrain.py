...
class BLIP_Pretrain(nn.Module):
    def __init__(self,                 
        ...
        self.pred_head = nn.Linear(text_width,len(self.tokenizer))
        self.mask_embed = nn.Parameter(torch.randn(vision_width))
        #self.prompt = nn.Embedding(9, text_width)
    ...
    def forward(self, image, caption, alpha):
        ...
        text_feat = F.normalize(self.text_proj(text_output.last_hidden_state[:,0,:]),dim=-1)# [b,256]                 
        ...
        ##================= MLM ========================##     
        masked_ids, text_mask = mask_text(text.input_ids)
        ...
        predict = self.pred_head(decoder_output.last_hidden_state[text_mask==1])
        loss_mlm = nn.CrossEntropyLoss()(predict,text.input_ids[text_mask==1])
        ...
        B = unmasked_patches.shape[0]
        self.visual_encoder.patch_embed.img_size = (16,16)
        unmask_tokens = self.visual_encoder.patch_embed(unmasked_patches.view(-1,3,16,16)).view(B,-1,768)
        self.visual_encoder.patch_embed.img_size = (224,224)
        cls_tokens = self.visual_encoder.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        unmask_tokens = torch.cat((cls_tokens, unmask_tokens), dim=1)
        pos_embed = torch.cat([self.visual_encoder.pos_embed[:,0:1,:].repeat(B,1,1),
                               self.visual_encoder.pos_embed[:,unmasked_idx+1,:].squeeze(0)],dim=1)
        unmask_tokens = unmask_tokens + pos_embed
        unmask_tokens = self.visual_encoder.pos_drop(unmask_tokens)
        
        for i,blk in enumerate(self.visual_encoder.blocks):
            unmask_tokens = blk(unmask_tokens)
        unmask_tokens = self.visual_encoder.norm(unmask_tokens)

        mask_tokens = self.mask_embed[None, None, :].repeat(B, masked_idx.shape[1], 1)
        mask_tokens += self.decoder_pos_embed(masked_idx)
        concat_tokens = torch.cat([mask_tokens, unmask_tokens], dim=1)
        ...
        recon_image_embeds = self.visual_decoder(encoder_embeds=dec_input_tokens,
                                                 ...
                                                 encoder_hidden_states = output_pos.last_hidden_state,
        ...
        return loss_ita, loss_itm, loss_mlm, loss_mim
